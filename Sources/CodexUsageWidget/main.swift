...
struct UsageWidgetView: View {
    ...
    private var usageOverviewSection: some View {
        let showsQuotaOverview = store.selectedRuntimeScope != .mimoCode
        return HStack(alignment: .center, spacing: 26) {
            if showsQuotaOverview {
                VStack(spacing: 8) {
                    DualQuotaRing(
            ...
            ForEach(ModelUsagePeriod.allCases) { item in
                Button {
                    ...
                    Text(item.label(language))
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 4, style: .continuous)
                                .fill(period == item ? visualTokens.selection.fill.color : Color.clear)
                        )
                }
                .buttonStyle(.plain)
            }
...
private struct ModelUsageHeader: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 4)
            header(language.text("速度", "TPS"), width: 40, alignment: .trailing, color: FixedVisualPalette.statusNeutral)
            header(language.text("未缓存", "Unc"), width: 42, alignment: .trailing, color: visualTokens.data.tokenInput.color)
            header(language.text("缓存", "Cache"), width: 42, alignment: .trailing, color: visualTokens.data.tokenCached.color)
            header(language.text("输出", "Out"), width: 42, alignment: .trailing, color: visualTokens.data.tokenOutput.color)
...
private struct ModelUsageRow: View {
    ...
    private var providerColor: Color {
        ...
        if model.contains("mimo") { return FixedVisualPalette.statusWarning }
        if model.contains("gpt") || model.contains("codex") || model.hasPrefix("o1") || model.hasPrefix("o3") { return FixedVisualPalette.statusSuccess }
        if model.contains("claude") { return visualTokens.accent.secondary.color }
        if model.contains("qwen") { return FixedVisualPalette.statusInfo }
        if model.contains("glm") { return visualTokens.accent.primaryLight.color }
        if model.contains("deepseek") { return FixedVisualPalette.statusDanger }
        if model.contains("gemini") { return FixedVisualPalette.statusSuccess }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            value(item.uncachedInputTokens, width: 42, color: visualTokens.data.tokenInput.color)
            value(item.cachedInputTokens, width: 42, color: visualTokens.data.tokenCached.color)
            value(item.outputTokens, width: 42, color: visualTokens.data.tokenOutput.color)
            value(item.tokens, width: 46, color: .primary, weight: .semibold)
            Text(cacheHitRate > 0 ? "\(Int(cacheHitRate))%" : "-")
                .font(.system(size: 11, weight: .medium, design: .rounded))
                .monospacedDigit()
                .foregroundStyle(cacheHitRate >= 50 ? visualTokens.accent.secondary.color : .secondary)
...
private func modelSpeedColor(_ tokensPerSecond: Double) -> Color {
    if tokensPerSecond >= 80 { return FixedVisualPalette.statusSuccess }
    if tokensPerSecond >= 40 { return FixedVisualPalette.statusInfo }
    if tokensPerSecond >= 20 { return FixedVisualPalette.statusWarning }
    return FixedVisualPalette.statusDanger
}

private func modelCurrencyColor(_ currency: ModelTokenCurrency) -> Color {
    currency == .cny ? FixedVisualPalette.statusDanger : FixedVisualPalette.statusSuccess
}
...
private struct CacheHitBadge: View {
    ...
    private var color: Color {
        ...
        if rate >= 0.85 { return FixedVisualPalette.statusSuccess }
        if rate >= 0.60 { return FixedVisualPalette.statusWarning }
        return FixedVisualPalette.statusDanger
    }
}
