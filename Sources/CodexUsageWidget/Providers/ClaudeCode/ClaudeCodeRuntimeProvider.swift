...
private final class ClaudeCodeTranscriptReader {
    ...
    private func makeLocalUsage(
        ...
        for delta in uniqueDeltas {
            ...
            let modelName = claudeModelName(delta.model)
            lifetime.add(tokens: delta.tokens, costUSD: cost)
            claudeAddModelUsage(tokens: delta.tokens, costUSD: cost, model: modelName, to: &lifetimeUsageByModel)
...
private func claudeModelName(_ model: String?) -> String {
    let value = (model ?? "unknown").trimmingCharacters(in: .whitespacesAndNewlines)
    let candidate = value.isEmpty ? "unknown" : value
    if candidate.count <= 28 { return candidate }
    return String(candidate.prefix(25)) + "..."
}

private func claudeAddModelUsage(
    tokens: TokenBreakdown,
    costUSD: Double,
    model: String,
    to values: inout [String: PricedTokenUsage]
) {
    var usage = values[model] ?? .zero
