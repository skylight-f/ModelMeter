...
codexU 是一个 macOS 菜单栏与桌面应用，用来查看 OpenAI Codex / ChatGPT Codex、Claude Code 和 MimoCode 的额度窗口、本机 token 用量和今日任务状态。它把常用信息放在菜单栏和主窗口里，帮助你快速判断剩余额度、重置时间和当天工作进展。
...
- 同时使用 Codex、Claude Code 或 MimoCode 做开发，希望在一个入口查看多端本机用量的人。
...
- 新增状态栏 Runtime 菜单：点击菜单栏图标后展示 Codex / Claude Code / MimoCode 卡片、可用额度、今日 token 与总 token。
...
- 支持 MimoCode 本机数据库用量、分模型 token 明细和基础任务看板；无法确认的模型价格显示为不可用，不伪造成 0。
- 模型用量支持今日、滚动 24H、近 7 天、本月、近 30 天和累计六个时间维度，可按模型或厂商搜索，并展示近 7 天模型构成趋势。
- 模型明细展示厂商、未缓存输入、缓存输入、输出、总 token、缓存率和本地估算价值；存在可靠完成时间时额外显示端到端估算速度，记录不足时保持不可用。
- 总览同步展示今日、滚动 24H、近 7 天、本月、近 30 天和累计 token 用量，并细分未缓存输入、命中缓存输入和输出。
...
- MimoCode 统计为可选能力；本机用量来自 `~/.local/share/mimocode/mimocode.db`，目前不提供官方额度窗口。
...
- MimoCode 本机 token 与任务：读取 `~/.local/share/mimocode/mimocode.db` 中的结构化 token 和 session 字段；未知模型价格保持不可用。
